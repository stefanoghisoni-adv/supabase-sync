...
describe('customer-consent', () => {
  describe('IDENTIFIER_COLUMNS', () => {
    it('includes id, email_address, phone_number, shopify_customer_id', () => {
      expect(IDENTIFIER_COLUMNS).toEqual(['id', 'email_address', 'phone_number', 'shopify_customer_id']);
    });
  });
  ...
  describe('isCustomerIdentifierLookup', () => {
    ...
    it('email_address=eq.x → true', () => {
      expect(isCustomerIdentifierLookup('?email_address=eq.foo@bar.com')).toBe(true);
    });
    it('shopify_customer_id=eq.1 → true', () => {
      expect(isCustomerIdentifierLookup('?shopify_customer_id=eq.1')).toBe(true);
    });
    it('phone_number=eq.+39 → true', () => {
      expect(isCustomerIdentifierLookup('?phone_number=eq.+390000')).toBe(true);
    });
  ...
  describe('consentCheckSearch', () => {
    it('preserva il filtro e imposta select=accepts_marketing', () => {
      const out = consentCheckSearch('?email_address=eq.foo@bar.com&select=*');
      const p = new URLSearchParams(out);
      expect(p.get('select')).toBe('accepts_marketing');
      expect(p.get('email_address')).toBe('eq.foo@bar.com');
    });

    it('rimuove limit e offset, preserva filtri', () => {
      const out = consentCheckSearch('?phone_number=eq.x&limit=1&offset=2');
      const p = new URLSearchParams(out);
      expect(p.get('select')).toBe('accepts_marketing');
      expect(p.get('phone_number')).toBe('eq.x');
